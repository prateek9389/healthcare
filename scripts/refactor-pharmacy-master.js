...
const files = walk(path.join(__dirname, '..', 'src', 'app', 'pharmacy'));
...
files.forEach(file => {
  ...
  // Fix check-in / specific green buttons if any exist in pharmacy
  ...
  if (content !== original) {
    ...
    console.log(`Updated pharmacy file: ${file}`);
  }
});
