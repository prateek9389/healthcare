...
import { 
  Calendar, CheckCircle2, Clock, XCircle, Eye,
  ChevronLeft, ChevronRight, Plus
} from 'lucide-react';

export default function AppointmentsPage() {
  const appointments = [
    { time: '09:30 AM', patient: 'Rahul Verma', doctor: 'Dr. Amit Mehta', dept: 'Cardiology', purpose: 'Consultation', status: 'Checked-In', statusColor: 'text-green-600 dark:text-green-400 bg-green-500/10 border-green-500/20' },
    { time: '10:00 AM', patient: 'Neha Sharma', doctor: 'Dr. Neha Sharma', dept: 'Physiotherapy', purpose: 'Follow-up', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { time: '10:30 AM', patient: 'Vikram Patel', doctor: 'Dr. Pooja Iyer', dept: 'Dermatology', purpose: 'Consultation', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { time: '11:00 AM', patient: 'Anita Singh', doctor: 'Dr. Amit Mehta', dept: 'Cardiology', purpose: 'ECG', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { time: '11:30 AM', patient: 'Suresh Kumar', doctor: 'Dr. Rajesh Gupta', dept: 'Orthopedics', purpose: 'Consultation', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { time: '12:00 PM', patient: 'Kavita Rani', doctor: 'Dr. Neha Sharma', dept: 'Physiotherapy', purpose: 'Therapy', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { time: '12:30 PM', patient: 'Amit Joshi', doctor: 'Dr. Amit Mehta', dept: 'Cardiology', purpose: 'Follow-up', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { time: '01:00 PM', patient: 'Pooja Verma', doctor: 'Dr. Pooja Iyer', dept: 'Dermatology', purpose: 'Consultation', status: 'Upcoming', statusColor: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
  ];
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-[20px] font-semibold text-foreground">Appointments</h1>
          <p className="text-sm text-foreground mt-1">Manage and schedule patient appointments</p>
        </div>
        
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-card border border-border-main rounded-lg px-3 py-2">
            <span className="text-sm text-foreground">Date</span>
            <Calendar className="w-3.5 h-3.5 text-foreground ml-1" />
            <span className="text-sm text-foreground ml-1">31 May 2025</span>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="card p-5 border-border-main bg-card shadow-lg flex items-center justify-between group">
          <div className="flex flex-col">
            <span className="text-sm font-bold text-foreground">Today's Appointments</span>
      ...
      <div className="card p-0 border-border-main bg-card shadow-lg flex flex-col">
        ...
        <div className="p-4 border-b border-border-main flex items-center gap-2">
          <button className="px-4 py-1.5 text-sm font-medium bg-[#2563eb] text-white rounded-md">All</button>
          <button className="px-4 py-1.5 text-sm font-bold text-foreground hover:text-foreground transition-colors">Upcoming</button>
            ...
            <tbody className="divide-y divide-border-main/50">
              {appointments.map((apt, i) => (
                <tr key={i} className="hover:bg-[#7caaaf] dark:hover:bg-white/5 transition-colors group">
                  <td className="p-4 pl-6 text-sm text-foreground">{apt.time}</td>
                  <td className="p-4 text-base font-medium text-foreground">{apt.patient}</td>
                  <td className="p-4 text-sm text-foreground">{apt.doctor}</td>
                  <td className="p-4 text-sm text-foreground">{apt.dept}</td>
                  <td className="p-4 text-sm text-foreground">{apt.purpose}</td>
                  <td className="p-4">
        ...
        <div className="p-4 border-t border-border-main flex items-center justify-between text-sm text-foreground bg-main">
          <span>Showing 1 to 8 of 56 appointments</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="w-6 h-6 rounded flex items-center justify-center hover:bg-[#7caaaf] dark:hover:bg-white/5 transition-colors">7</button>
            <button className="w-6 h-6 rounded flex items-center justify-center text-foreground hover:text-foreground transition-colors">
